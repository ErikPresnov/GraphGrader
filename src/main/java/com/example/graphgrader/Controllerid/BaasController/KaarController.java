...
public class KaarController {
    ...
    public Button lock;
    public Button lae;
    public boolean suunatud = false;
    public boolean kruskal = false;

    public void laeGraaf(MouseEvent ignored) {
        try {
            controller.g = new Graaf("Graafid\\test2.txt", true);
        } catch (IOException e) {
    ...
    public Arrow addArrowHander(Arrow arrow, Tipp algus, Tipp lopp, Kaar kaar) {
        arrow.setOnMouseClicked(e -> {
            if (algus == praegune && lopp.seis != TipuSeis.TÖÖDELDUD && lopp.seis != TipuSeis.PRAEGUNE) {
                ...
                pseudoStruktuur.getChildren().add(new Text(lopp.tähis + "\t"));
    ...
    public void showGraph() {
        for (int i = 0; i < controller.g.tipud.size(); i++) {
            ...
            if (i == 0) {
                praegune = tipp;
                praegune.muudaSeisu(TipuSeis.PRAEGUNE);
            } else
    ...
    public void LockGraph(MouseEvent ignored) {
        ...
        if (kruskal) {
            for (Tipp tipp : controller.g.tipud)
                for (Kaar kaar : tipp.kaared) controller.lisa(kaar);
        }
    }

    public void uuendaAndmed(MinHeapKaared kuhi) {
        pseudoStruktuur.getChildren().clear();
        for (int i = 0; i < kuhi.heap.size(); i++)
            pseudoStruktuur.getChildren().add(new Text(kuhi.heap.get(i).toString()));
    }
}
